 ...
 * Date : 2020/5/18
 * Time : 10:42
 * Introduction : Zoomable 是 Fresco Demo 工程里自带的图片缩放库
 * <p>
 * github 地址：https://github.com/facebook/fresco/tree/master/samples/zoomable
 * <p>
 * add ziwen :
 * 提供方法保存缩放倍数，以达到当前 View 重新可见时缩放倍数不被重置的目的
 */
public class ZoomableDraweeView extends DraweeView<GenericDraweeHierarchy>
     ...
     * add ziwen
     * 是否保存缩放倍数
     * 为true时当前 view 重新可见时缩放倍数不会被重置
     */
    private boolean mIsSaveScale = false;

    private void onRelease() {
        FLog.v(getLogTag(), "onRelease: view %x", this.hashCode());
        //add ziwen 拦截 reset ，使得当前 view 重新可见时缩放倍数不会被重置
        if (!mIsSaveScale) {
            mZoomableController.setEnabled(false);
        }
    }
     ...
     * 是否保存缩放倍数
     *
     * @param saveScale 为true时当前 view 重新可见时缩放倍数不会被重置，默认false
     */
    public void setSaveScale(boolean saveScale) {
        mIsSaveScale = saveScale;
    }
}
